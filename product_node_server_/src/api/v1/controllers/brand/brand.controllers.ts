...
export const createBrand = async (req: Request, res: Response) => {
  try {
    ...
    const existingBrand = await BrandModel.findOne({ brand_name:newBrand.brand_name });
    if (existingBrand) {
      return res.status(404).json({
        message: MESSAGE.custom(`Brand with the name "${newBrand.brand_name}" already exists.`) ,
...
export const updateBrand = async (req: Request, res: Response) => {
  try {
    ...
    const updatedBrand = await BrandModel.findByIdAndUpdate(brandId, req.body, { new: true });
    if (!updatedBrand) {
      return res.status(404).json({
