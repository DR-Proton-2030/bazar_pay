...
const createProductByAdmin = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        if (!subCategoryInstance) {
            return res.status(http_status_codes_1.StatusCodes.NOT_FOUND).json({
                message: message_1.MESSAGE.custom("Sub Category not found!")
            });
        }
        ...
        if (String(productPayload.category_object_id) !== String(subCategoryInstance.category_object_id)) {
            return res.status(http_status_codes_1.StatusCodes.NOT_FOUND).json({
                message: message_1.MESSAGE.custom("Category not matched with sub category!")
            });
        }
        const brandInstance = yield brand_model_1.default.findOne({ _id: productPayload.brand_object_id });
        if (!brandInstance) {
            return res.status(http_status_codes_1.StatusCodes.NOT_FOUND).json({
                message: message_1.MESSAGE.custom("Brand not found!")
            });
        }
        ...
        const productInstancePayload = Object.assign(Object.assign({}, productPayload), { product_image: productUrl, category_object_id: subCategoryInstance.category_object_id, brand_object_id: brandInstance._id, subcategory_object_id: subCategoryInstance._id });
